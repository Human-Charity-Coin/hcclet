...
humancharitycoin = (require 'node-humancharitycoin')()
humancharitycoin.auth(app.get('humancharitycoin_username'),app.get('humancharitycoin_password')).set('host', 'localhost').set({port:9335})

app.set 'name', 'hcclet'
app.set 'admin_email', "support@cryptoloverbots.com"
app.set 'email_name', 'HCCLet'
...
app.get '/', (req, res)->
  ...
  if email and req.session.address # If we're logged in properly...
    ...
    humancharitycoin.getBalance email, (err, result)->
...
app.get '/signup', (req,res)->
  ...
  humancharitycoin.getAddressesByAccount email, (err, result)->
    ...
    if result.length
      ...
      mailer.send_mail
          to: email
          subject: "Your HCCLet (humancharitycoin wallet)"
          body:"""
          Here's your secure HCCLet link:
        ...
        , (success, message)->
          ...
          return security_error res, 'You already have a wallet. Check your email (search inbox for hcclet.com)'

    else
      humancharitycoin.getNewAddress email, (err, result)->
        ...
        mailer.send_mail
            to: email
            subject: "Your New HCCLet (humancharitycoin wallet)"
            body:"""
            Here's your secure HCCLet link:
          ...
          , (success, message)->
            ...
            return security_error res, 'Your new wallet has been created! Check your email (search inbox for HCCLet.com)'
...
app.get '/login', (req,res)->
  ...
  if get_email_hash(req.query.email) is req.query.hash # hash is good.
    ...
    humancharitycoin.getAddressesByAccount req.query.email, (err, result)->
...
app.get '/send', login_required, (req,res)->
  ...
  humancharitycoin.validateAddress req.query.send_to, (err, result)->
    if err
      return security_error '' + err
    if result.isvalid
      humancharitycoin.sendFrom email, req.query.send_to, parseFloat(req.query.amount), 1, ->
        res.send arguments
...
app.get '/history', login_required, (req,res)->
  ...
  humancharitycoin.listTransactions req.session.email, 11, page * 0, (err, transactions)->
    if err
      return security_error '' + err
    res.render 'history',
...
app.get '/history/:tid', login_required, (req,res)->
  ...
  humancharitycoin.gettransaction req.params.tid, (err, transaction)->
    if err
      return security_error '' + err
